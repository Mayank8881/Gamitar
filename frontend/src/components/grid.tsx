...
const Grid: React.FC<GridProps> = ({ grid, selectedCell, onCellSelect, hasSubmitted }) => {
  const handleCellClick = (row: number, col: number) => {
    if (hasSubmitted) return
    if (grid[row][col]) return
    onCellSelect(row, col)
  ...
  return (
    <div className="inline-block bg-card border border-border rounded-lg p-4 shadow-lg">
      <div className="grid gap-1" style={{ gridTemplateColumns: "auto repeat(10, 1fr)" }}>
        {/* Top-left corner */}
        ...
        {Array.from({ length: 10 }, (_, col) => (
          <div
            key={`col-header-${col}`}
            className="w-10 h-10 flex items-center justify-center text-muted-foreground font-semibold text-xs bg-muted rounded"
          >
            {col + 1}
          </div>
        ))}
          ...
          <React.Fragment key={`row-${rowIndex}`}>
            ...
            {row.map((cell, colIndex) => {
              ...
              return (
                <div
                  key={`${rowIndex}-${colIndex}`}
                  className={`w-10 h-10 flex items-center justify-center rounded font-semibold text-sm transition-all duration-200 cursor-pointer ${
                    isSelected
                      ? "bg-accent text-accent-foreground border-2 border-accent scale-110 shadow-lg shadow-accent/50"
                      : isOccupied
                        ? "bg-muted text-muted-foreground border border-border cursor-not-allowed"
                        : "bg-input border border-border hover:bg-muted hover:border-accent hover:scale-105"
                  } ${hasSubmitted && !isOccupied ? "cursor-not-allowed opacity-50" : ""}`}
                  onClick={() => handleCellClick(rowIndex, colIndex)}
                  title={
                    isOccupied
                      ? `Occupied: ${cell}`
